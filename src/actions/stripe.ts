...
export const getAllProductsFromStripe = async () => {
  try {
    ...
    // 🛠️ TEMPORARILY COMMENTED: User connection check ko bypass kiya testing ke liye
    /*
    ...
    */
    ...
    // 🚀 FIXED: Ab ye aapke main Stripe account se direct products laayega
    const products = await stripe.products.list({
      active: true, // Sirf active products laaye ga
    })
...
return {
  ...
  customerId: customer.id,
    ...
    console.log('failed to load subscription error', error);
    return {
      status: 400,
